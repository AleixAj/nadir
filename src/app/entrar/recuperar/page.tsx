...
export default function RecuperarPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    setSending(true);
    const { error } = await authClient.requestPasswordReset({ email: email.trim(), redirectTo: "/entrar/nueva-contrasena" });
    setSending(false);
    ...
    <AuthFrame title="¿Has olvidado tu contraseña?" subtitle="Te enviaremos un enlace para elegir una nueva.">
      {sent ? (
        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-3">
          ...
          <button type="submit" disabled={sending} className={btn("primary", "lg", "h-11 w-full text-sm")}>
